...
must_haves:
  truths:
    - "Module page shows + icons to add and X icons to remove members in draft mode"
    - "User can add/remove module members and sees 'Saving...' indicator during save"
    - "Bundle page shows + icon to add and X icons to remove modules in draft mode"
    - "User can add/remove bundle modules and sees 'Saving...' indicator during save"
  artifacts:
...
This gap closure adds edit functionality that was specified in 13-06 but not fully implemented during execution.
...
**Migration note:** The current implementation uses inline EditableField for simple text editing. This task replaces it with the EntityHeader component pattern used by CategoryDetail, which provides consistent styling and edit mode behavior across all detail pages.
...
8. **Replace current inline EditableField usage with EntityHeader component** (migrating from current pattern):
```typescript
<EntityHeader
  entityKey={entityKey}
  label={editedLabel}
