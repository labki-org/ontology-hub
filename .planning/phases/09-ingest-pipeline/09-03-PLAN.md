...
must_haves:
  truths:
    - "Ingest replaces all canonical data atomically in one transaction"
    - "Deletion order respects FK constraints (relationships first, then entities)"
    - "Relationships are resolved from entity_key to UUID after entities are flushed"
    - "Materialized view is refreshed in separate transaction after main commit"
    - "OntologyVersion tracks commit SHA, status, counts, warnings, and errors"
  artifacts:
    - path: "backend/app/services/ingest_v2.py"
      provides: "IngestService class and sync_repository_v2 function"
      exports: ["IngestService", "sync_repository_v2"]
      min_lines: 150
  key_links:
    - from: "ingest_v2.py"
      to: "app.services.validators.SchemaValidator"
      via: "import and instantiation"
      pattern: "from app.services.validators import SchemaValidator"
    - from: "ingest_v2.py"
      to: "app.services.parsers.EntityParser"
      via: "import and instantiation"
      pattern: "from app.services.parsers import EntityParser"
    - from: "ingest_v2.py"
      to: "app.models.v2.*"
      via: "Model imports for database operations"
      pattern: "from app.models.v2 import"
    - from: "ingest_v2.py"
      to: "refresh_category_property_effective"
      via: "Mat view refresh call"
      pattern: "refresh_category_property_effective"
---

<objective>
Create the core v2.0 ingest service that fetches repo content, validates schemas, parses entities, and atomically replaces all canonical data.

Purpose: This is the heart of Phase 9 - ING-02, ING-04, ING-05, ING-06, ING-07. It orchestrates validation, parsing, atomic replacement, and mat view refresh.
Output: IngestService class and sync_repository_v2() function that can be called from webhook handler.
</objective>
...
@.planning/phases/09-ingest-pipeline/09-01-SUMMARY.md
@.planning/phases/09-ingest-pipeline/09-02-SUMMARY.md
...
<task type="auto">
  <name>Task 1: Create IngestService class</name>
  <files>backend/app/services/ingest_v2.py</files>
  <action>
Create the v2.0 ingest service with the following structure:
...
from app.models.v2 import (
    # Entities
    Bundle, Category, Module, Property, Subobject, Template,
    # Relationships
    BundleModule, CategoryParent, CategoryProperty, ModuleEntity,
    # Version tracking
    OntologyVersion, IngestStatus, EntityType,
    # Mat view refresh
...
python -c "from app.services.ingest_v2 import IngestService; print('OK')"
```
  </verify>
  <done>IngestService class created with all helper methods</done>
</task>

<task type="auto">
  <name>Task 2: Add sync_repository_v2 function</name>
  <files>backend/app/services/ingest_v2.py</files>
  <action>
Add the main entry point function that orchestrates the full ingest:
...
```bash
cd /home/daharoni/dev/ontology-hub/backend
python -c "from app.services.ingest_v2 import sync_repository_v2; print('OK')"
```
  </verify>
  <done>sync_repository_v2() function orchestrates full ingest flow</done>
</task>

</tasks>

<verification>
1. `from app.services.ingest_v2 import IngestService, sync_repository_v2` succeeds
2. IngestService has all methods: load_schemas, load_entity_files, delete_all_canonical, insert_entities, resolve_and_insert_relationships, refresh_mat_view
3. sync_repository_v2 imports and uses SchemaValidator and EntityParser
4. Deletion order is correct: relationships -> entities -> OntologyVersion
5. OntologyVersion created with commit_sha, status, counts, warnings
</verification>

<success_criteria>
- IngestService class orchestrates repo fetching and database operations
- Atomic replacement: delete all in FK order, insert all, within single transaction
- Two-phase resolution: entities flushed, then relationships resolved by key lookup
- Mat view refreshed in separate transaction after main commit
- OntologyVersion tracks commit SHA, status, entity counts, warnings/errors
- Validation errors abort ingest and create FAILED OntologyVersion
</success_criteria>

<output>
After completion, create `.planning/phases/09-ingest-pipeline/09-03-SUMMARY.md`
</output>
