...
must_haves:
  ...
  artifacts:
    ...
    - path: "frontend/src/api/entitiesV2.ts"
      ...
      exports: ["useCategories", "useCategory", "useProperties", "useModules", "useBundles", "useOntologyVersion"]
  key_links:
...
- useOntologyVersion() - fetch current ontology version (commit_sha, ingested_at) from /api/v2/ontology-version
...
Follow existing pattern from entities.ts but target v2 endpoints.
  ...
  <done>All 9 hooks created (8 entity hooks + useOntologyVersion), types exported, draft_id param flows through to API calls</done>
</task>
