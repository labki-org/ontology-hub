...
Structure:
1. Header with "Ontology Hub" link to / and version display
   - Fetch current ontology version using a new useOntologyVersion() hook from entitiesV2.ts
   - Display commit SHA (truncated to 7 chars) in muted text below title
   - Example: "Ontology Hub" on first line, "v abc1234" in smaller muted text below
...
- Current ontology version (commit SHA) displays in sidebar header (FE-04)
