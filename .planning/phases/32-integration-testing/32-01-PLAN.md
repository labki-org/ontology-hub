...
must_haves:
  truths:
    - "Dashboard CREATE change produces PR file at dashboards/{key}.json"
    - "Resource CREATE change produces PR file at resources/{key}.json"
    - "Dashboard/Resource DELETE changes produce file deletion markers"
    - "UPDATE changes apply patch and produce correct file content"
  artifacts:
    - path: "backend/app/services/pr_builder.py"
      provides: "Dashboard and Resource entity support"
      contains: "dashboard.*resources"
    - path: "backend/tests/test_pr_builder.py"
      provides: "PR file structure tests"
      min_lines: 80
  key_links:
    - from: "backend/app/services/pr_builder.py"
      to: "ENTITY_DIRS"
      via: "entity type mapping"
      pattern: "dashboard.*resources"
---

<objective>
Add Dashboard and Resource entity support to PR builder and verify file structure in tests.

Purpose: Fulfill INTG-03 (PR submission includes dashboard/resource files) by ensuring the PR builder correctly generates file paths and content for dashboard and resource entity changes.

Output: Updated pr_builder.py with dashboard/resource support, new test file test_pr_builder.py with file structure verification tests.
</objective>
...
@.planning/PROJECT.md
@.planning/ROADMAP.md
@.planning/REQUIREMENTS.md (INTG-03)

# Key source files
@backend/app/services/pr_builder.py
@backend/tests/conftest.py
@backend/tests/test_draft_crud_dashboard_resource.py (fixture patterns)
...
<task type="auto">
  <name>Task 1: Add Dashboard and Resource to PR Builder</name>
  <files>backend/app/services/pr_builder.py</files>
  <action>
Update pr_builder.py to support Dashboard and Resource entities:

1. Add import for Dashboard, Resource from app.models.v2:
   ```python
...
3. Add to ENTITY_DIRS dict:
   ...
   ENTITY_DIRS = {
       ...
       "bundle": "bundles",
       "template": "templates",
       "dashboard": "dashboards",  # ADD
       "resource": "resources",    # ADD
   }
   ```

Note: Resources use hierarchical paths like templates (e.g., "Equipment/Microscope"), but the PR builder already handles "/" in entity_key by extracting the last segment as filename. This produces "resources/Microscope.json" which matches the research finding about flattened paths.
  </action>
  <verify>
Verify the changes:
```bash
grep -A10 "ENTITY_MODELS" backend/app/services/pr_builder.py | grep -E "(dashboard|resource)"
grep -A10 "ENTITY_DIRS" backend/app/services/pr_builder.py | grep -E "(dashboard|resource)"
```
Both should return matches.
  </verify>
  <done>
pr_builder.py includes dashboard and resource in both ENTITY_MODELS and ENTITY_DIRS mappings.
  </done>
</task>

<task type="auto">
  <name>Task 2: Create PR Builder Tests</name>
  <files>backend/tests/test_pr_builder.py</files>
  <action>
Create new test file for PR builder file structure verification. Follow patterns from test_draft_crud_dashboard_resource.py.
...
Important: Use existing fixture patterns from conftest.py (test_session, test_engine). Do NOT duplicate the test_engine/test_session fixtures - they're already defined in conftest.py.
  </action>
  <verify>
Run the new tests:
```bash
cd /home/daharoni/dev/ontology-hub/backend && python -m pytest tests/test_pr_builder.py -v
```
All tests should pass.
  </verify>
  <done>
test_pr_builder.py exists with tests for dashboard and resource PR file generation. All tests pass.
  </done>
</task>
...
<verification>
After both tasks:
1. pr_builder.py has dashboard and resource in ENTITY_MODELS and ENTITY_DIRS
2. test_pr_builder.py tests pass verifying correct file paths and content
3. No import errors or regressions
</verification>

<success_criteria>
- Dashboard CREATE produces "dashboards/{key}.json"
- Resource CREATE produces "resources/{key}.json" (flattened from hierarchical key)
- UPDATE changes apply patches and produce correct content
- DELETE changes produce deletion markers
- All tests pass
</success_criteria>

<output>
After completion, create `.planning/phases/32-integration-testing/32-01-SUMMARY.md`
</output>
