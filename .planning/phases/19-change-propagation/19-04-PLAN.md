...
4. The `category.parents` field already contains parent entity_keys. Build an inheritance chain display:
   ```typescript
   // Simple display: just show direct parents with edit highlighting
   // For deeper ancestry, we'd need to fetch parent details recursively
   // Start simple: show parents array with highlighting
   ```
...
5. Add new AccordionSection after the "Parent Categories" section:
   ...
   {editedParents.length > 0 && (
     ...
     >
       ...
       {editedParents.some((p) => directEdits.has(p)) && (
...
TypeScript compiles: `cd frontend && npx tsc --noEmit`
  ...
  <done>
CategoryDetail shows Inheritance Chain section with clickable parent links, edited parents highlighted with blue background and "edited" badge.
  </done>
</task>
