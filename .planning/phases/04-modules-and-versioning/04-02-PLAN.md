...
must_haves:
  ...
  key_links:
    - from: "frontend/src/pages/ProfilePage.tsx"
      to: "DependencyGraph"
      via: "component import, passes modules from useProfileModules"
      pattern: "<DependencyGraph.*modules="
    - from: "frontend/src/pages/ModulePage.tsx"
      to: "/api/modules/{id}/overlaps"
      via: "useModuleOverlaps hook, passes result to ModuleEntityList"
...
4. **Create `frontend/src/components/graph/DependencyGraph.tsx`**:
   - Props: `modules: ModulePublic[]` (full module objects, NOT just IDs)
   - Extract edges from each module's `dependencies` field:
     - For each module, create edge from each dependency module_id TO that module
     - Edge direction: dependency -> dependent (arrow shows "depends on")
   - Create nodes from the modules array (label, entityCount from category_ids.length)
   ...
   interface DependencyGraphProps {
     modules: ModulePublic[]      // Full module objects with dependencies field
...
8. **Update `frontend/src/pages/ProfilePage.tsx`**:
   - ProfilePage already uses `useProfileModules(profileId)` which returns `ModulePublic[]`
   - Pass the full modules array to DependencyGraph: `<DependencyGraph modules={modules} />`
   - DependencyGraph extracts dependency edges from each module's `dependencies` field
