...
<task type="auto">
  <name>Task 2: Add get changes endpoint for validation context</name>
  <files>backend/app/routers/drafts_v2.py</files>
  <action>
Add endpoint to get draft changes summary (needed for diff view and PR body generation):

```python
from app.models.v2 import DraftChange
from app.schemas.draft_change import DraftChangesListResponse, DraftChangeResponse


@router.get("/{token}/changes", response_model=DraftChangesListResponse)
@limiter.limit(RATE_LIMITS["draft_read"])
async def get_draft_changes(
    ...
    """Get all changes in a draft.

    Returns all DraftChange records for the draft, ordered by creation time.
    Used for displaying diff view and generating PR body.
...
Note: This duplicates the endpoint in draft_changes.py but uses the token directly in the URL pattern (drafts/{token}/changes vs drafts/{token}/changes from draft_changes router). The original endpoint in draft_changes.py is mounted at /api/v2/drafts/{token}/changes already, so this may not be needed. Check if the existing endpoint is accessible and skip this if so.

Actually, looking at the draft_changes.py router, it's already mounted. So this task should just verify the existing endpoint works or add any missing imports to make validation work.
  </action>
  <verify>cd /home/daharoni/dev/ontology-hub && python -c "from app.routers.drafts_v2 import validate_draft, get_draft; print('Both endpoints exist')"</verify>
  <done>Draft changes endpoint is accessible via /api/v2/drafts/{token}/changes</done>
</task>
